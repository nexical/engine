...
// Constants for types (imported statically only for type usage if possible, but here we need runtimes)
// To keep it simple, we use the dynamically imported classes.
...
describe('Workspace', () => {
    ...
    describe('savePlan', () => {
        it('should save plan to disk', async () => {
            ...
            expect(mockFileSystem.writeFileAtomic).toHaveBeenCalledWith('plan_current', expect.stringContaining('plan_name: test plan'));
        });
    });
    ...
    describe('detectSignal', () => {
        ...
        it('should handle parse error in signal file', async () => {
            ...
            expect(signal).toBeNull();
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Failed to parse signal file'), expect.anything());
            consoleSpy.mockRestore();
        });
    });
    ...
    describe('async write error handling', () => {
        it('should log error if write fails', async () => {
            mockFileSystem.writeFileAtomic.mockImplementation(() => { throw new Error('Write failed'); });
            const consoleSpy = jest.spyOn(console, 'error').mockImplementation(() => { });
