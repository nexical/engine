...
describe('Project', () => {
    ...
    describe('getConfig', () => {
        ...
        it('should log error and throw if loading fails', () => {
            mockFileSystem.exists.mockReturnValue(true);
            mockFileSystem.readFile.mockImplementation(() => { throw new Error('Read failed'); });

            const consoleSpy = jest.spyOn(console, 'error').mockImplementation(() => { });

            expect(() => project.getConfig()).toThrow('Read failed');
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Failed to load project profile'), expect.any(Error));
