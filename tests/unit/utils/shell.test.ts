...
describe('ShellExecutor', () => {
    ...
    describe('execute', () => {
        it('should execute command successfully', async () => {
            ...
            const calls = (mockChild.on as jest.Mock).mock.calls;
            const closeHandler = calls.find(c => c[0] === 'close')[1] as Function;
            closeHandler(0);
        ...
        it('should handle stdout/stderr data', async () => {
            ...
            // Simulate stdout
            const stdoutHandler = (mockChild.stdout.on as jest.Mock).mock.calls.find(c => c[0] === 'data')[1] as Function;
            stdoutHandler(Buffer.from('out'));

            // Simulate stderr
            const stderrHandler = (mockChild.stderr.on as jest.Mock).mock.calls.find(c => c[0] === 'data')[1] as Function;
            stderrHandler(Buffer.from('err'));

            const closeHandler = (mockChild.on as jest.Mock).mock.calls.find(c => c[0] === 'close')[1] as Function;
            closeHandler(0);
        ...
        it('should handle process error', async () => {
            ...
            const errorHandler = (mockChild.on as jest.Mock).mock.calls.find(c => c[0] === 'error')[1] as Function;
