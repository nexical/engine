...
jest.unstable_mockModule('../../../src/workflow/states/ArchitectingState.js', () => ({ ArchitectingState: MockArchitectingState }));
jest.unstable_mockModule('../../../src/workflow/states/PlanningState.js', () => ({ PlanningState: MockPlanningState }));
jest.unstable_mockModule('../../../src/workflow/states/ExecutingState.js', () => ({ ExecutingState: MockExecutingState }));
jest.unstable_mockModule('../../../src/workflow/states/CompletedState.js', () => ({ CompletedState: MockCompletedState }));
...
describe('Workflow', () => {
    ...
    it('should use default maxLoops if not provided in config', async () => {
        ...
        await workflow.start(engineState);
        expect(mockHost.log).toHaveBeenCalledWith('error', expect.stringContaining('Maximum retry limit reached (10 loops)'));
    });
