...
import type { CLISkill as CLISkillType } from '../../../src/skills/CLISkill.js';
...
const { CLISkill } = await import('../../../src/skills/CLISkill.js');

describe('CLISkill', () => {
    let cliSkill: CLISkillType;
    ...
    beforeEach(() => {
        ...
        cliSkill = new CLISkill(mockOrchestrator);
        mockShellExecutor.execute.mockReset();
    });

    it('should implement isSupported returning true', () => {
        expect(cliSkill.isSupported({ binaries: {} })).toBe(true);
    });
    ...
    it('should execute a CLI command successfully', async () => {
        ...
        const result = await cliSkill.execute(agent, 'task prompt', {
            userPrompt: 'World',
            taskId: 'task-1'
        });
    ...
    it('should throw on execution error', async () => {
        ...
        await expect(cliSkill.execute(agent, 'task prompt')).rejects.toThrow('Execution failed');
    });
    ...
    it('should throw on non-zero exit code', async () => {
        ...
        await expect(cliSkill.execute(agent, 'task prompt')).rejects.toThrow('Command exited with code 1');
    });
